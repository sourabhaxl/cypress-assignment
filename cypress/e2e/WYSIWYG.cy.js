...
it('Verifies that the text is bold after clicking bold button',()=>{
...
it('Verifies if the text is aligned right',()=>{
    ...
    cy.get('.note-editable > p').clear().type('Test text {selectall}')
     ...
     cy.get('[data-original-title="Align right (⌘+⇧+R)"] > .note-icon-align-right').click({force:true})
 ...
 it.only('Updates the background color and verify the color',()=>{
   ...
   cy.get('.note-editable > p').clear().type('Test text {selectall}')
    ...
    cy.get(':nth-child(1) > .note-holder > .note-color-palette > :nth-child(2) > [style="background-color:#FF0000"]').click({force:true})
